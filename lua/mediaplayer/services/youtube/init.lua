...
---  
-- [FALLBACK 2] Use metadata gathered client-side by the iframe prefetch  
-- (received via NetReadRequest). Last resort in the fallback chain.  
--  
function SERVICE:UsePrefetchedMetadata( callback )  
	if not self._metaTitle then  
		callback( false, "YouTube prefetch fallback: no prefetched title available" )  
		return  
	end  
...
---  
-- Stepped fallback chain:  
--   cache -> JSON API -> HTML scrape -> iframe prefetch  
...
function SERVICE:GetMetadata( callback )  
	...
	-- Debug override: bypass the API and go straight to HTML scraping.  
	if self.ForceHTMLScraping then  
		self:FetchHTMLMetadata(function( metadata, htmlReason )  
			...
			MsgN( "[MediaPlayer] " .. tostring(htmlReason) .. " — falling back to iframe prefetch." )  
			self:UsePrefetchedMetadata( callback )  
		end)  
		return  
	end  
	...
	self:FetchAPIMetadata(function( metadata, apiReason )  
		...
		-- Step 2: server-side HTML scrape  
		self:FetchHTMLMetadata(function( metadata2, htmlReason )  
			...
			-- Step 3: client-side iframe prefetch result (last resort)  
			self:UsePrefetchedMetadata( callback )  
		end)  
	end)  
end  
